...
- **Gewicht (Stand Gen 58):** index.html 40.954 Bytes = 39,99 KB — direkt an der Grenze. **Gen 59 beginnt mit Straffung.** Ausgelagert sind bereits: Selbst-Vorstellung (wer-ich-bin.html), Wortschatz + Marksteine (worte.json). Nächste Kandidaten: die Gruß-Sätze (Gen 22) und die Atem-/Fokus-Texte — nur mit Notlicht im Körper und sw.js CORE.
...
- **Gewicht (Stand Gen 52):** 38.936 Bytes = 38,0 KB von 40 KB. Erledigt in Gen 52: CSS-Variable `--licht` statt sieben Verläufen, `say()`/`calm()` statt zwanzig Wiederholungen, storyFallback in Zehnerschritten (−1,8 KB). **Verbleibende Reserve für später:** Selbst-Vorstellung als nachgeladene Datei (~2 KB; dann sw.js CORE ergänzen, sonst leidet das Offline-Herz).
